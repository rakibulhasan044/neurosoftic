...
export async function CollectionGrid() {
  let collections = [];
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_API_URL || 'http://localhost:8000/api/v1'}/collections`, { cache: 'no-store' });
    if (res.ok) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-auto md:h-[600px]">
          ...
          <div className="relative rounded-2xl overflow-hidden group h-[400px] md:h-full">
            {firstCollection.image ? (
              <img src={firstCollection.image} alt={firstCollection.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
            ) : (
            ...
            <div className="absolute bottom-0 left-0 p-8 w-full">
              <h3 className="text-3xl font-bold text-white mb-3">{firstCollection.name}</h3>
              <p className="text-white/80 mb-6 max-w-sm">{firstCollection.description || 'Discover our curated selection.'}</p>
              <Link href={`/collections/${firstCollection.slug}`} className="inline-flex items-center text-white font-medium hover:underline">
          ...
          <div className="flex flex-col gap-6 h-[800px] md:h-full">
            {rightCollections.map((col: any) => (
              <div key={col.id} className="relative rounded-2xl overflow-hidden group flex-1">
                {col.image ? (
                ...
                <div className="absolute bottom-0 left-0 p-8 w-full">
                  ...
                  <Link href={`/collections/${col.slug}`} className="inline-flex items-center text-white font-medium hover:underline">
