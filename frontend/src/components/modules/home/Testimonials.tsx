...
export async function Testimonials() {
  let reviews = [];
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_API_URL || 'http://localhost:8000/api/v1'}/reviews`, { cache: 'no-store' });
    if (res.ok) {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.slice(0, 3).map((review: any, i: number) => (
            <Card key={review.id || i} className="bg-background border-border/50 shadow-sm hover:shadow-md transition-shadow">
